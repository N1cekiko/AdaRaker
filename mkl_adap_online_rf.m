...
for t=	1:T     
    ...
    x=X(:,t);
    for k=1:n_act
        eta(k)=min(0.1/S,.1/sqrt(S^k));
        if  mod(t-1,S^(k-1))==0 
            params.eta=min(0.1/S,.1/sqrt(S^k));
            act_w(k)=params.eta;
...
%                 end
           ...
           % params.theta_ini=ones(n_ker,1)/n_ker;
        ...
        [w_temp,f_temp,theta_temp]=run_mkl_online_rf(yt,D,x,w,params);
